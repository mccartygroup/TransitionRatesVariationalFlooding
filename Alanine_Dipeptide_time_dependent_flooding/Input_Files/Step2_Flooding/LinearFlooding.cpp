...
//+PLUMEDOC BIAS LINEARFLOODING
/*

*/
//+ENDPLUMEDOC

class LinearFlooding : public Bias {
private:
    unsigned int ncv_;
    double temp_;
    double kB_temp_;
...
public:
  explicit LinearFlooding(const ActionOptions&);
  ~LinearFlooding();
  void calculate();
  static void registerKeywords(Keywords& keys);
};

PLUMED_REGISTER_ACTION(LinearFlooding,"LINEARFLOODING")

void LinearFlooding::registerKeywords(Keywords& keys) {
...
LinearFlooding::~LinearFlooding(){

}

LinearFlooding::LinearFlooding(const ActionOptions&ao):
PLUMED_BIAS_INIT(ao)
{
...
double LinearFlooding::get_max_value(vector<double> y_data) {
...
double LinearFlooding::get_FF_value(double vs, double Vmax){
  return -1.0*vs + Vmax;
}

// get switching function
double LinearFlooding::get_fermi_value(double Fs, double a, double lambda){
  return 1.0 / (1.0 + exp(lambda*(Fs - a)));
}

// get derivative of switching function
double LinearFlooding::get_fermi_derivative(double Fs, double a, double lambda, double dydx){
  double f1;
  double f2;
  f1 = exp(lambda*(Fs-a));
